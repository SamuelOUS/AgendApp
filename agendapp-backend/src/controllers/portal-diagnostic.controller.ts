...
import { fetchAffiliateData } from '../infraestructure/browser/session.service.js';
...
export async function affiliateDataHandler(req: Request, res: Response): Promise<void> {
  try {
    const data = await fetchAffiliateData(req.activeSession!.page);
    ...
    res.status(500).json({
      ok: false,
      error: err instanceof Error ? err.message : 'No fue posible consultar datos del afiliado',
    });
  }
}
